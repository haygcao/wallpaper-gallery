# Fork 部署指南

本文档为 Fork 本项目的用户提供完整的部署配置指南。

## 📋 目录

- [快速开始](#快速开始)
- [环境变量配置](#环境变量配置)
- [Supabase 数据库配置](#supabase-数据库配置)
- [GitHub Actions 配置](#github-actions-配置)
- [网站分析配置](#网站分析配置)
- [Cloudflare 配置](#cloudflare-配置)
- [自定义图床配置](#自定义图床配置)

---

## 快速开始

### 1. Fork 项目

点击 GitHub 页面右上角的 Fork 按钮，将项目 Fork 到你的账户。

### 2. 克隆到本地

```bash
git clone https://github.com/你的用户名/wallpaper-gallery.git
cd wallpaper-gallery
pnpm install
```

### 3. 本地开发

```bash
# 生成壁纸数据（会自动从线上拉取）
pnpm generate

# 启动开发服务器
pnpm dev
```

### 4. 部署

推送到 `main` 分支后，GitHub Actions 会自动构建并部署到 GitHub Pages。

---

## 环境变量配置

项目使用 `.env.*` 文件管理环境变量。Fork 后需要修改以下文件：

### `.env.development` (开发环境)

```bash
# 环境标识
VITE_ENV=development
VITE_SHOW_ENV_BADGE=true

# Umami Analytics Website ID（可选，留空则不启用）
VITE_UMAMI_WEBSITE_ID=你的开发环境Website_ID

# Supabase 配置（可选，留空则不启用统计功能）
VITE_SUPABASE_URL=你的Supabase项目URL
VITE_SUPABASE_ANON_KEY=你的Supabase匿名Key
```

### `.env.production` (生产环境)

```bash
# 环境标识
VITE_ENV=production
VITE_SHOW_ENV_BADGE=false

# Umami Analytics Website ID（可选）
VITE_UMAMI_WEBSITE_ID=你的生产环境Website_ID

# Supabase 配置（可选）
VITE_SUPABASE_URL=你的Supabase项目URL
VITE_SUPABASE_ANON_KEY=你的Supabase匿名Key
```

> **注意**：如果不需要统计功能，可以将 Supabase 和 Umami 相关配置留空或删除。

---

## Supabase 数据库配置

Supabase 用于记录壁纸的浏览量和下载量统计。**此功能为可选**，不配置也不影响网站正常使用。

### 1. 创建 Supabase 项目

1. 访问 [Supabase](https://supabase.com/) 并注册账号
2. 创建新项目，选择离你最近的区域
3. 等待项目初始化完成

### 2. 获取项目凭证

在 Supabase 控制台中：
- **Project Settings** → **API**
- 复制 `Project URL` → 填入 `VITE_SUPABASE_URL`
### 2. 获取项目凭证

在 Supabase 控制台中：
- **Project Settings** → **API**
- 复制 `Project URL` → 填入 `VITE_SUPABASE_URL`
- 复制 `anon public` Key → 填入 `VITE_SUPABASE_ANON_KEY`

### 3. 配置 GitHub Secrets（重要）

为了保护你的 Supabase 凭证，需要在 GitHub 仓库中配置 Secrets：

1. 进入你的 GitHub 仓库
2. 点击 **Settings** → **Secrets and variables** → **Actions**
3. 点击 **New repository secret** 添加以下 Secrets：

| Secret 名称 | 值 | 说明 |
|------------|-----|------|
| `VITE_SUPABASE_URL` | 你的 Supabase Project URL | 例如：https://xxx.supabase.co |
| `VITE_SUPABASE_ANON_KEY` | 你的 Supabase anon public key | 从 Supabase 控制台复制 |

**为什么要用 Secrets？**
- ✅ 防止 API Key 泄露
- ✅ 避免别人 Fork 你的项目后消耗你的 Supabase 配额
- ✅ 每个 Fork 用户需要配置自己的 Supabase

**注意**：
- 如果不配置这些 Secrets，部署会成功，但统计功能不可用
- Fork 用户必须配置自己的 Supabase 才能启用统计功能

### 4. 创建数据库表

在 Supabase 控制台的 **SQL Editor** 中执行以下 SQL：

```sql
-- ============================================
-- 创建统计表 image_stats
-- ============================================
CREATE TABLE IF NOT EXISTS public.image_stats (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  image_id TEXT NOT NULL UNIQUE,        -- 图片唯一标识（filename）
  series TEXT NOT NULL,                 -- 系列：desktop, mobile, avatar, bing
  category TEXT,                        -- 分类
  total_views INT DEFAULT 0,            -- 总浏览量
  total_downloads INT DEFAULT 0,        -- 总下载量
  last_viewed_at TIMESTAMPTZ,
  last_downloaded_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- 创建索引
CREATE INDEX IF NOT EXISTS idx_image_stats_image_id ON image_stats(image_id);
CREATE INDEX IF NOT EXISTS idx_image_stats_series ON image_stats(series);
CREATE INDEX IF NOT EXISTS idx_image_stats_views ON image_stats(series, total_views DESC);
CREATE INDEX IF NOT EXISTS idx_image_stats_downloads ON image_stats(series, total_downloads DESC);

-- ============================================
-- 创建原子递增函数
-- ============================================

-- 浏览计数函数
CREATE OR REPLACE FUNCTION increment_view(
  img_id TEXT,
  series_name TEXT,
  cat TEXT DEFAULT NULL
) RETURNS VOID AS $$
BEGIN
  INSERT INTO image_stats (image_id, series, category, total_views, last_viewed_at, updated_at)
  VALUES (img_id, series_name, cat, 1, NOW(), NOW())
  ON CONFLICT (image_id) DO UPDATE SET
    total_views = image_stats.total_views + 1,
    last_viewed_at = NOW(),
    updated_at = NOW();
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 下载计数函数
CREATE OR REPLACE FUNCTION increment_download(
  img_id TEXT,
  series_name TEXT,
  cat TEXT DEFAULT NULL
) RETURNS VOID AS $$
BEGIN
  INSERT INTO image_stats (image_id, series, category, total_views, total_downloads, last_downloaded_at, updated_at)
  VALUES (img_id, series_name, cat, 0, 1, NOW(), NOW())
  ON CONFLICT (image_id) DO UPDATE SET
    total_downloads = image_stats.total_downloads + 1,
    last_downloaded_at = NOW(),
    updated_at = NOW();
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- ============================================
-- 配置 RLS 权限
-- ============================================

-- 启用 RLS
ALTER TABLE image_stats ENABLE ROW LEVEL SECURITY;

-- 允许匿名用户读取
CREATE POLICY "Allow anonymous read" ON image_stats
  FOR SELECT
  USING (true);

-- ============================================
-- 创建导出热门数据的函数（供 GitHub Actions 使用）
-- ============================================
CREATE OR REPLACE FUNCTION get_hot_stats(
  series_filter TEXT,
  limit_count INT DEFAULT 500
) RETURNS TABLE (
  image_id TEXT,
  total_views INT,
  total_downloads INT
) AS $$
BEGIN
  RETURN QUERY
  SELECT
    i.image_id,
    i.total_views,
    i.total_downloads
  FROM image_stats i
  WHERE i.series = series_filter
  ORDER BY i.total_views DESC
  LIMIT limit_count;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
```

### 4. 验证配置

执行以下 SQL 验证表是否创建成功：

```sql
SELECT * FROM image_stats LIMIT 10;
```

---

## GitHub Actions 配置

### 必需的 Secrets

在 GitHub 仓库的 **Settings** → **Secrets and variables** → **Actions** 中添加：

| Secret 名称 | 说明 | 是否必需 |
|------------|------|---------|
| `CLOUDFLARE_ZONE_ID` | Cloudflare 区域 ID | 可选（用于缓存清除） |
| `CLOUDFLARE_API_TOKEN` | Cloudflare API Token | 可选（用于缓存清除） |
| `VITE_SUPABASE_URL` | Supabase 项目 URL | 可选（用于统计导出） |
| `VITE_SUPABASE_ANON_KEY` | Supabase 匿名 Key | 可选（用于统计导出） |

### 工作流说明

| 工作流 | 文件 | 触发条件 | 功能 |
|-------|------|---------|------|
| Deploy | `deploy.yml` | 推送到 main / 每日定时 / 手动 | 构建并部署到 GitHub Pages |
| Export Stats | `export-stats.yml` | 每日定时 / 手动 | 导出热门统计数据 |

### 启用 GitHub Pages

1. 进入仓库 **Settings** → **Pages**
2. Source 选择 **GitHub Actions**
3. 首次部署后会自动生成访问地址

---

## 网站分析配置

### Umami Analytics（推荐）

[Umami](https://umami.is/) 是一个开源的网站分析工具，注重隐私保护。

#### 配置步骤

1. 注册 [Umami Cloud](https://cloud.umami.is/) 或自建 Umami
2. 添加网站，获取 Website ID
3. 将 Website ID 填入 `.env.production` 的 `VITE_UMAMI_WEBSITE_ID`

#### 自建 Umami

如果你想自建 Umami，可以参考 [Umami 官方文档](https://umami.is/docs/getting-started)。

### Cloudflare Web Analytics（可选）

如果你的网站使用 Cloudflare，可以启用免费的 Web Analytics：

1. 登录 Cloudflare Dashboard
2. 进入 **Analytics & Logs** → **Web Analytics**
3. 添加站点并获取 JS 代码
4. 将代码添加到 `index.html`

---

## Cloudflare 配置

如果你使用 Cloudflare 作为 CDN，可以配置自动缓存清除。

### 获取 Zone ID

1. 登录 Cloudflare Dashboard
2. 选择你的域名
3. 在 **Overview** 页面右侧找到 **Zone ID**

### 创建 API Token

1. 进入 **My Profile** → **API Tokens**
2. 点击 **Create Token**
3. 使用 **Edit zone DNS** 模板或自定义权限
4. 确保包含 **Zone** → **Cache Purge** → **Purge** 权限
5. 限制 Zone 为你的域名

### 添加到 GitHub Secrets

- `CLOUDFLARE_ZONE_ID`: Zone ID
- `CLOUDFLARE_API_TOKEN`: API Token

---

## 自定义图床配置

默认情况下，项目使用作者的图床数据。如果你想使用自己的图床：

### 1. 准备图床仓库

创建一个 GitHub 仓库存放壁纸图片，目录结构：

```
your-wallpaper-repo/
├── wallpaper/
│   ├── desktop/          # 电脑壁纸
│   │   ├── 分类1/
│   │   └── 分类2/
│   ├── mobile/           # 手机壁纸
│   │   └── ...
│   └── avatar/           # 头像
│       └── ...
└── thumbnail/            # 缩略图（可选）
    ├── desktop/
    ├── mobile/
    └── avatar/
```

### 2. 修改 CDN 配置

编辑 `src/utils/constants.js`：

```javascript
// 替换为你的图床地址
export const CDN_BASE_URL = 'https://cdn.jsdelivr.net/gh/你的用户名/你的图床仓库@main'
export const CDN_VERSION = 'main'  // 或使用 tag 版本
```

### 3. 修改数据生成脚本

编辑 `scripts/generate-data.js`：

```javascript
// 替换为你的线上数据源（如果有）
const ONLINE_DATA_BASE_URL = 'https://你的域名/data'
```

### 4. 修改 GitHub Actions

编辑 `.github/workflows/deploy.yml`，将图床仓库改为你的：

```yaml
- name: Checkout wallpaper source
  uses: actions/checkout@v4
  with:
    repository: 你的用户名/你的图床仓库
    path: nuanXinProPic
```

---

## 常见问题

### Q: 不配置 Supabase 会怎样？

A: 网站可以正常运行，只是没有浏览量/下载量统计功能，热门排序也不会生效。

### Q: 不配置 Umami 会怎样？

A: 网站可以正常运行，只是没有访问统计分析功能。

### Q: 不配置 Cloudflare 会怎样？

A: 网站可以正常运行，只是部署后不会自动清除 CDN 缓存，可能需要等待缓存过期。

### Q: 如何使用自己的域名？

A: 
1. 在仓库根目录创建 `CNAME` 文件，内容为你的域名
2. 在域名 DNS 中添加 CNAME 记录指向 `你的用户名.github.io`
3. 在 GitHub Pages 设置中配置自定义域名

### Q: 构建失败怎么办？

A: 检查以下几点：
1. 确保 `pnpm-lock.yaml` 已提交
2. 检查 GitHub Actions 日志中的错误信息
3. 确保 Secrets 配置正确（如果使用了相关功能）

---

## 技术支持

如有问题，欢迎提交 [Issue](https://github.com/IT-NuanxinPro/wallpaper-gallery/issues)。
